...
const ShowItem = () => {
  ...
  return (
    <Container className="mt-5 ">
      <h2 className="fw-semibold fs-1 mb-2">{item.name}</h2>

      <Image
        src={item.image_url}
        alt={item.name}
        className="mb-2 w-h-353 d-block  mx-auto"
        fluid
      />

      <Row className="justify-content-between mt-4">
        <Col className="col-12 col-md-6">
          <h4 className="fw-semibold fs-50 w-max-content">
            Price:
      ...
      <Row className="mt-4 text-center mt-5">
        <Col className=" ">
          <h4 className="fw-semibold fs-50 w-max-content  mx-auto">
            Updated At:
            <span className="text-secondary fw-medium fs-1">
              {new Intl.DateTimeFormat("en-GB").format(
